...
import { useParams } from "react-router-dom";
import { db } from "../../firebaseConfig";
import { collection, getDocs } from "firebase/firestore"; // 👈 dynamic filtering සඳහා getDocs භාවිතය
import {
  ResponsiveContainer,
  PieChart,
  Pie,
...
const AdminDashboard = () => {
  const { subject } = useParams(); // 👑 URL එකෙන් එන විෂය (maths / science / english)
  const [totalStudents, setTotalStudents] = useState(0);
  const [genderData, setGenderData] = useState([
    { name: "Boys", value: 0 },
    { name: "Girls", value: 0 },
  ...
  // 🗓️ Static Weekly Schedules (ඉදිරියේදී පන්තිය අනුව dynamic කළ හැක)
  const weeklySchedules = [
    {
      day: "Monday",
      time: "04:00 PM - 06:00 PM",
      class: `Grade 10 ${subject?.toUpperCase()}`,
    },
    {
      day: "Wednesday",
      time: "03:30 PM - 06:00 PM",
      class: `Grade 11 ${subject?.toUpperCase()}`,
    },
    {
      day: "Friday",
      time: "04:00 PM - 07:00 PM",
    ...
    },
    {
      day: "Sunday",
      time: "08:00 AM - 12:00 PM",
      class: `${subject?.toUpperCase()} Master Revision`,
    },
  ];
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      setIsLoading(true);
      try {
        // 📡 1. Firebase එකෙන් මුළු ශිෂ්‍ය ලේඛනයම මුලින්ම ඇදලා ගැනීම [INDEX 51]
        const studentSnapshot = await getDocs(collection(db, "students"));
        const allStudents = studentSnapshot.docs.map((doc) => doc.data());

        // 🔍 2. සැබෑ Content Gating Logic (විෂය අනුව ළමයි වෙන් කිරීම) [INDEX 4]
        // Maths -> 'M', Science -> 'S', English -> 'E' ලෙස ID එකේ මැද අකුර පරික්ෂා කරයි
        const currentSubjectLetter = subject?.toUpperCase().charAt(0) || "M";

        const filteredStudents = allStudents.filter((student) => {
          const cleanStudentId = student.id?.toUpperCase() || "";
          return cleanStudentId.includes(`-${currentSubjectLetter}-`);
        });

        // 🔢 3. විෂයට අදාළ මුළු ශිෂ්‍ය සංඛ්‍යාව සෙට් කිරීම
        setTotalStudents(filteredStudents.length);

        // 👦👧 4. ඒ වෙන් කරගත් ළමයි අතරින් විතරක් Boys/Girls ප්‍රමාණය ගණනය කිරීම
        let boysCount = 0;
        let girlsCount = 0;

        filteredStudents.forEach((student) => {
          const cleanGender = student.gender?.toLowerCase() || "";
          if (cleanGender === "male" || cleanGender === "boy") {
            boysCount++;
          } else {
            girlsCount++;
          }
        ...
        // 🍩 චාට් එකට අදාළ දත්ත යාවත්කාලීන කිරීම
        setGenderData([
          { name: "Boys", value: boysCount },
          { name: "Girls", value: girlsCount },
        ]);
      } catch (error) {
        console.error("Dashboard multi-tenant filtering error:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <div
        ...
        Streaming Isolated Faculty Metrics from Cloud Ledger...
      </div>
    );
  }
  ...
  return (
    <div
      ...
      <div
        ...
        <p style={{ margin: "5px 0 0 0", opacity: 0.8, fontSize: "0.95rem" }}>
          Live isolated overview, subject-gated distributions, and core
          operations .
        </p>
      </div>
      {/* 🎛️ MIDDLE LAYER */}
      <div
        ...
        <div
          ...
          <div
            ...
            <span
              ...
              Active {subject?.toUpperCase()} Students
            </span>
          </div>
          <h2
            style={{
          ...
          <p style={{ margin: 0, fontSize: "0.85rem", color: "#8b949e" }}>
            Subscribers securely gated for this specific faculty environment
        .
          </p>
        </div>
        ...
        <div
          ...
          <h3
            ...
            <FaChartPie style={{ color: "#ff4bb4" }} /> {subject?.toUpperCase()}{" "}
            Gender Distribution
          </h3>
      ...
      <div
        ...
        <h3
          ...
          <FaCalendarDays style={{ color: "#0056ff" }} /> Weekly Faculty
          Timelines
        </h3>
