...
import { collection, getDocs, query, where } from "firebase/firestore"; // ☁️ Cloud එකෙන් දත්ත සොයන Tools ගත්තා

const Login = () => {
  const [studentId, setStudentId] = useState("");
  const [password, setPassword] = useState("");
  const [error, setError] = useState("");
  const [isLoggingIn, setIsLoggingIn] = useState(false); // 🕒 බටන් එක ලෝඩ් වන විට පෙන්වීමට
  const navigate = useNavigate();
  ...
  // 🧠 ☁️ [THE CLOUD AUTH ENGINE]: ළමයා ගහපු ID එක සජීවීව Google Cloud එකෙන් සොයා ලොග් කරන හැටි
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const querySnapshot = await getDocs(q); // 🚀 ඔන්න සැබෑ Cloud GET (getDocs) එක!
  ...
  return (
    <div
      ...
      <h2 style={{ color: "#1a0a54", marginBottom: "5px" }}>
        Student Login Portal
      </h2>
      <p style={{ fontSize: "0.85rem", marginBottom: "20px", color: "#666" }}>
        Enter your approved student ID and Password to enter the dashboard.
      </p>
      ...
      <form
        ...
        <div className="input-group">
          ...
          <input
            ...
            required
            value={password}
            onChange={(e) => setPassword(e.target.value)} // 💡 👑 FIXED: පරණ කැඩුණු onChange ලොජික් එක පිරිසිදු කළා මචං
            style={{
              width: "100%",
              padding: "10px",
              borderRadius: "8px",
        ...
        <button
          ...
          {isLoggingIn ? "⏳ Processing Auth..." : "Access Dashboard"}
        </button>
      </form>
    </div>
  );
