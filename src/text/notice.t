...
const ClassNoticeVault = () => {
  // 🛠️ බාහිරින් ප්‍රොප්ස් ගන්නවා වෙනුවට 6-11 පන්ති තෝරන්න ඇතුළතින්ම State එකක් හැදුවා
  ...
  // 📡 selectedGrade එක වෙනස් වන හැම වෙලාවකම Firebase එකෙන් ඒ පන්තියේ දත්ත Live Sync වේ [INDEX 51]
  useEffect(() => {
    setIsLoading(true); // පන්තිය මාරු කරද්දී Loading තත්ත්වයට පත් කිරීම
    const q = query(
  ...
  return (
    <div className="vault-container">
      ...
      {/* 🗂️ 6 - 11 හ 11 Paper Class තෝරාගැනීමට හැදූ නවීන GRADE SELECTOR OPTION එක */}
      <div className="global-vault-grade-selector-bar" style={{ marginBottom: "25px", background: "#f8faff", padding: "15px", borderRadius: "10px", border: "1px solid #eef2ff" }}>
        <label style={{ fontWeight: "bold", color: "#001b42", marginRight: "12px", fontSize: "0.95rem" }}>🎯 Target Audience / Select Grade Class:</label>
        <select 
          value={selectedGrade} 
          onChange={(e) => setSelectedGrade(e.target.value)}
          style={{ padding: "8px 12px", borderRadius: "6px", border: "1px solid #c7d2fe", background: "white", fontWeight: "600", color: "#001b42", cursor: "pointer" }}
        >
          <option value="6">Grade 6 Class</option>
          <option value="7">Grade 7 Class</option>
          <option value="8">Grade 8 Class</option>
          <option value="9">Grade 9 Class</option>
          <option value="10">Grade 10 Class</option>
          <option value="11">Grade 11 Theory/Revision</option>
          <option value="11-Paper">Grade 11 Premium Paper Class</option> {/* 👈 11 Paper Class එකට විශේෂ Option එකක් දැම්මා */}
        </select>
      </div>
      ...
      <div className="notice-vault-grid">
        ...
        <form
          ...
          {/* 🛠️ තෝරාගත් ශ්‍රේණිය ස්වයංක්‍රීයව වෙනස් වේ */}
          <h3>Broadcast to Grade {selectedGrade === "11-Paper" ? "11 Paper Class" : selectedGrade}</h3>
        ...
        <div className="vault-live-feed">
          <h3>Active Grade {selectedGrade === "11-Paper" ? "11 Paper Class" : selectedGrade} Notices</h3>

          {isLoading ? (
            <div className="vault-loading">Streaming Live Cloud Vault...</div>
            ) : notices.length === 0 ? (
            <div className="vault-empty">
              No announcements published for Grade {selectedGrade === "11-Paper" ? "11 Paper Class" : selectedGrade} yet.
            </div>
          ) : (
            <div className="vault-scroll-list">
              {notices.map((item) => (
                <div key={item.docId} className="notice-card"> {/* 🛠️ Key එක docId ලෙස නිවැරදි කර ඇත */}
                  ...
                  <div className="notice-footer">
                    ...
                    <button
                      onClick={() => handleDeleteNotice(item.docId)} // 🛠️ id වෙනුවට docId ලෙස නිවැරදි කළා
